...
const label: CSSProperties = { fontSize: 13, fontWeight: 600, color: 'var(--text-2)', display: 'grid', gap: 6 };
...
export function RoutineBuilder({ initialMachines = [], displayName = null, initialExperience = 'beginner' }: {
  ...
  const machineList = () => machines.split(',').map((s) => s.trim()).filter(Boolean);
  ...
  const submit = () => { setRes(null); generate(machineList()); };

  return (
    <section style={{ display: 'grid', gap: 14, padding: '58px 22px 0' }}>
      <div>
        <div style={{ fontSize: 13, fontWeight: 600, letterSpacing: 1.2, textTransform: 'uppercase', color: 'var(--text-3)' }}>
          오늘의 코치
        </div>
      ...
      <label style={label}>
        내 머신 (쉼표로 구분)
        <input value={machines} onChange={(e) => setMachines(e.target.value)} placeholder="펙덱, 랫풀다운, 레그프레스" style={field} />
      </label>
      <div style={{ display: 'flex', gap: 10 }}>
        ...
        <label style={{ ...label, width: 130 }}>
          경험
          <select value={experience} onChange={(e) => setExperience(e.target.value as Experience)} style={field}>
            {EXPERIENCES.map((x) => <option key={x.value} value={x.value}>{x.label}</option>)}
          </select>
        </label>
      </div>
      <button onClick={submit} disabled={loading} style={{ ...primaryBtn, opacity: loading ? 0.6 : 1 }}>
        {loading ? '생성 중…' : '루틴 만들기'}
      </button>
              ...
              return (
                <div key={ex.exerciseId} style={{
                  ...
                  <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 8 }}>
                    <span style={{
                      fontSize: 18, fontWeight: 750, color: 'var(--accent)', letterSpacing: -0.3,
                    }}>{load}</span>
...
function AliasForm({ alias, onRegistered }: { alias: string; onRegistered: () => void }) {
  ...
  const sel: CSSProperties = {
    height: 40, borderRadius: 12, border: '1px solid var(--hair)', background: 'var(--bg-elev2)',
    color: 'var(--text)', padding: '0 10px', fontSize: 14,
  };
  return (
    <div style={{ border: '1px solid var(--accent)', background: 'var(--accent-soft)', borderRadius: 'var(--r-card)', padding: 14 }}>
      ...
      <div style={{ display: 'flex', gap: 8, marginTop: 10, flexWrap: 'wrap' }}>
        ...
        <button onClick={register} disabled={!exerciseId || busy} style={{
          height: 40, padding: '0 16px', borderRadius: 12, border: 'none', cursor: 'pointer',
          background: 'var(--accent)', color: 'var(--accent-ink)', fontSize: 14, fontWeight: 650,
          opacity: !exerciseId || busy ? 0.5 : 1,
        }}>{busy ? '등록 중…' : '등록'}</button>
      </div>
    </div>
  );
    ...
    <div style={{ display: 'grid', gap: 8, marginTop: 4 }}>
      <label style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-2)', display: 'grid', gap: 6 }}>
        ...
        <textarea value={question} onChange={(e) => setQuestion(e.target.value)} rows={2}
          placeholder="예: 레그프레스 발 위치는? / 초보자는 주당 며칠?"
          style={{
            width: '100%', borderRadius: 'var(--r-btn)', border: '1px solid var(--hair)', background: 'var(--bg-elev)',
            color: 'var(--text)', padding: '10px 14px', fontSize: 15, outline: 'none', resize: 'vertical',
          }} />
      </label>
      <button onClick={ask} disabled={busy} style={{
        height: 48, borderRadius: 'var(--r-btn)', border: '1px solid var(--accent)', background: 'transparent',
        color: 'var(--accent)', fontSize: 15, fontWeight: 650, cursor: 'pointer', opacity: busy ? 0.6 : 1,
      }}>{busy ? '근거 찾는 중…' : '왜? (논문 근거 찾기)'}</button>
      {data && (
